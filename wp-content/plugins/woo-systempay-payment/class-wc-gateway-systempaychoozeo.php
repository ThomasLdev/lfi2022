...
class WC_Gateway_SystempayChoozeo extends WC_Gateway_SystempayStd
{
    ...
    {
        $this->id = 'systempaychoozeo';
        $this->icon = apply_filters('woocommerce_systempaychoozeo_icon', WC_SYSTEMPAY_PLUGIN_URL . 'assets/images/choozeo.png');
        $this->has_fields = true;
        $this->method_title = self::GATEWAY_NAME. ' - ' . __('Choozeo payment', 'woo-systempay-payment');
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields['enabled']['description'] = __('Enables / disables Choozeo payment.', 'woo-systempay-payment');

        $this->form_fields['title']['default'] = __('Payment with Choozeo without fees', 'woo-systempay-payment');
        ...
        $this->form_fields['multi_options'] = array(
            'title' => __('CHOOZEO PAYMENT OPTIONS', 'woo-systempay-payment'),
            'type' => 'title'
        );
        ...
        $this->form_fields['payment_options'] = array(
            ...
            'description' => __('Define amount restriction for each card.', 'woo-systempay-payment')
        );
    }
        ...
        foreach ($options as $key => $option) {
            ...
            if (count($options) == 1) {
                echo '<input type="hidden"
                             id="systempaychoozeo_option_' . $key . '"
                             value="' . $key . '"
                             name="systempaychoozeo_option">';
            } else {
            ...
            echo '<label for="systempaychoozeo_option_' . $key . '" style="display: inline;">' . '
    ...
    public function process_payment($order_id)
    {
        ...
        $option = $_POST['systempaychoozeo_option'];
        $options = $this->get_available_options();
        $label = $options[$option]['label'];

        // Save selected payment option into session...
        set_transient('systempaychoozeo_option_' . $order_id, $option);
    ...
    protected function systempay_fill_request($order)
    {
        ...
        $option = get_transient('systempaychoozeo_option_' . self::get_order_property($order, 'id'));
        $this->systempay_request->set('payment_cards', $option);
        ...
        $this->systempay_request->set('cust_country', 'FR');

        delete_transient('systempaychoozeo_option_' . self::get_order_property($order, 'id'));
    }
}

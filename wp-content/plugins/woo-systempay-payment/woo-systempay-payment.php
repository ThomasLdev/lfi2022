...
function woocommerce_systempay_init()
{
    ...
    require_once 'includes/SystempayRequest.php';
    require_once 'includes/SystempayResponse.php';
    require_once 'includes/SystempayRest.php';
...
function woocommerce_systempay_add_method($methods)
{
    ...
    if (version_compare($woocommerce->version, '2.3.0', '>=') && $woocommerce->cart) {
        $regrouped_other_payments = new WC_Gateway_SystempayRegroupedOther();
        ...
        if (! $regrouped_other_payments->regroup_other_payment_means()) {
            ...
            if (is_array($payment_means) && ! empty($payment_means)) {
                foreach ($payment_means as $option) {
                    $methods[] = new WC_Gateway_SystempayOther($option['payment_mean'], $option['label']);
                }
            }
        }
    }
...
function woocommerce_systempay_order_payment_gateways($available_gateways)
{
    ...
    // User created systempay not grouped other payment means lets reorder payment gatways as they appear in woocommerce backend.
...
if (! function_exists('ly_saved_cards_link')) {
    function ly_saved_cards_link($menu_links)
    {
        ...
        + array('ly_saved_cards' => __('My payment means', 'woo-systempay-payment'))
...
if (! function_exists('ly_change_saved_cards_title')) {
    ...
    {
        return __('My payment means', 'woo-systempay-payment');
    }
...
function systempay_my_account_endpoint_content()
{
    global $woocommerce;

    $cust_id = WC_Gateway_Systempay::get_customer_property($woocommerce->customer, 'id');

    $sub_module_saving_cards_ids = array('systempaystd', 'systempaysubscription');
    ...
    if (! empty($customer_saved_cards)) {
        ...
        echo '<table id="ly_cards_table" class="shop_table" id ="systempay-customer-card" style="display:none">
                <thead>
                  <tr>';
        if ($column_card_brand) {
            echo '<th>' . __('Type', 'woo-systempay-payment') . '</th>';
        }

        echo      '<th>' . __('Means of payment', 'woo-systempay-payment') . '</th>
                  <th>' . __('Action', 'woo-systempay-payment') . '</th>
                  </tr>
                </thead>
                <tbody>
                </tbody>
        ...
        foreach ($customer_saved_cards as $id => $card) {
            ...
            if ($column_card_brand) {
                ...
                $remote_logo = WC_Gateway_Systempay::LOGO_URL . strtolower($card['card_brand']) . '.png';
                if ($card['card_brand']) {
            ...
            $table_body .= '<td><a href=\"javascript: void(0);\" onclick=\"systempayConfirmDelete(\'' . $id . '\')\">' . __('Delete', 'woo-systempay-payment') . '</a></td></tr>';
        }

        $delete_card_url = add_query_arg('wc-api', 'WC_Gateway_Systempay_Delete_Saved_Card', home_url('/'));
        ...
        echo '<script>
                  ...
                  function systempayConfirmDelete(id) {
                      if (confirm("' . __('Are you sure you want to delete your saved means of payment? This action is not reversible!', 'woo-systempay-payment') . '")) {
                          jQuery("body").block({
                             message: null,
                             overlayCSS: {
                                 background: "#fff",
        ...
        echo '<div id="ly_empty_cards_message" class="woocommerce-Message woocommerce-Message--info woocommerce-info" style="display:none">'
                  . __('You have no stored payment means.', 'woo-systempay-payment') .
             '</div>
             <script>
                 jQuery(document).ready(function(){
...
add_action('woocommerce_account_ly_saved_cards_endpoint', 'systempay_my_account_endpoint_content');

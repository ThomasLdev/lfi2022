...
class SystempayRestTools
{
    public static function convertRestResult($answer)
    {
        ...
        $response['vads_currency'] = SystempayApi::getCurrencyNumCode(self::getProperty($transaction, 'currency'));
        ...
        if ($transactionDetails = self::getProperty($transaction, 'transactionDetails')) {
            ...
            $effectiveCurrency = SystempayApi::getCurrencyNumCode(self::getProperty($transactionDetails, 'effectiveCurrency'));
    ...
    public static function checkResult($response, $expectedStatuses = array())
    {
        ...
        if ($response['status'] != 'SUCCESS') {
            ...
            require_once 'SystempayRestException.php';
            throw new SystempayRestException($errorMessage, $answer['errorCode']);
        } elseif (! empty($expectedStatuses) && ! in_array($answer['detailedStatus'], $expectedStatuses)) {
            throw new Exception(sprintf(__('Unexpected transaction type received (%s).', 'woo-systempay-payment'), $answer['detailedStatus']));
        }
    }
}
